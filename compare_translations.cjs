...
// Show some sample keys from each file for debugging
console.log('\n=== SAMPLE DUTCH KEYS ===');
[...nlKeys].sort().slice(0, 10).forEach(key => console.log(`- ${key}`));

console.log('\n=== SAMPLE FRENCH KEYS ===');
[...frKeys].sort().slice(0, 10).forEach(key => console.log(`- ${key}`));