...
const ReferralSignupModal: React.FC<ReferralSignupModalProps> = ({ isOpen, onClose, onEnroll, t }) => {
  ...
  const handleGenerate = async () => {
    ...
    if (!validatePayPalMeLink(paypalLink)) {
      setIsValid(false);
      setError(t('referralPaypalInvalid', 'Ongeldige PayPal.Me link. Gebruik het formaat paypal.me/naam'));
      return;
    }
    setGenerating(true);
    try {
      ...
      if (e?.message?.includes('rate-limit-exceeded') || e?.status === 429) {
        ...
        setError(t('referralRateLimitError', `Te veel pogingen. Je kunt over ${retryMinutes} minuten opnieuw proberen. Dit voorkomt spam en misbruik van het systeem.`));
      } else {
        setError(e?.message || t('referralEnrollError', 'Er ging iets mis bij aanmelden. Probeer opnieuw.'));
      }
    } finally {
      setGenerating(false);
    }
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/50 backdrop-blur-sm" role="dialog" aria-modal="true">
      <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-2xl shadow-2xl max-w-2xl w-full p-6 sm:p-7">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">{t('referralSignupTitle', 'Aanmelden voor Referral Programma')}</h2>
        ...
        <p className="text-sm mb-3">
          {t('referralPaypalMeDesc', 'PayPal.Me is een persoonlijke betaal-link. Deel je link met anderen; zij kunnen je geld sturen zonder emailadres of telefoonnummer.')} 
          <a className="underline text-blue-600 dark:text-blue-400" href={PAYPAL_ME_INFO_URL} target="_blank" rel="noreferrer">
            {t('referralPaypalLearnMore', 'Volledige uitleg')}
          </a>
        </p>

        <label className="block text-sm font-medium mb-1" htmlFor="paypalLink">{t('referralPaypalMeLabel', 'PayPal.Me link')}</label>
        <input
          id="paypalLink"
          type="text"
          value={paypalLink}
        ...
        />
        {!isValid && (
          <div className="text-red-600 text-sm mt-1">{t('referralPaypalInvalid', 'Ongeldige PayPal.Me link. Gebruik het formaat paypal.me/naam')}</div>
        )}
        ...
        {!generated && (
          <div className="mt-5 flex justify-end gap-3">
            ...
            <button onClick={handleGenerate} disabled={generating || !isValid || !paypalLink} className="px-4 py-2 rounded-md bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-50">
              {generating ? t('generating', 'Genereren...') : t('referralGenerateCode', 'Code genereren en aanmelden')}
            </button>
          </div>
        )}
          ...
          <div className="mt-6 p-6 rounded-lg bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 border-2 border-green-300 dark:border-green-600">
            ...
            <div className="mb-4 p-3 rounded-lg bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-600">
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm font-medium text-yellow-800 dark:text-yellow-200">
                  {t('referralOneTimeWarning', 'BELANGRIJK: Deze code wordt slechts EENMALIG verstrekt! Kopieer en bewaar deze zorgvuldig.')}
                </div>
              </div>
            </div>
            ...
            <div className="text-center mb-4">
              <div className="text-lg font-bold text-gray-700 dark:text-gray-300 mb-2">
                {t('referralYourCode', 'Jouw referral code')}
              </div>
              <div className="relative">
                ...
                <button
                  ...
                  title={t('copyCode', 'Code kopiëren')}
                >
                  📋
                </button>
              </div>
            ...
            <div className="mt-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => navigator.clipboard.writeText(generated.joinUrl)}
            ...
            <div className="mt-4 p-3 rounded-lg bg-blue-100 dark:bg-blue-900/30 border border-blue-300 dark:border-blue-600">
              <div className="text-sm text-blue-800 dark:text-blue-200">
                <div className="font-medium mb-1">
                  {t('referralInstructions', 'Instructies:')}
                </div>
                <ul className="list-disc list-inside space-y-1 text-xs">
                  <li>{t('referralInstruction1', 'Deel deze code met mensen die zich willen aanmelden')}</li>
                  <li>{t('referralInstruction2', 'De code kan maar één keer worden gebruikt')}</li>
                  <li>{t('referralInstruction3', 'Bewaar de code op een veilige plaats')}</li>
                </ul>
              </div>
            </div>
          </div>
